...
bot.on("message", async (msg) => {
  ...
  if (msg.audio) {
    ...
    const redirectLink = `https://shared-redirect.onrender.com/pay?bot=xposure&recipient=${TREASURY.toBase58()}&amount=0.01&reference=${reference}&userId=${userId}`;
...
bot.on("callback_query", async (q) => {
  try {
    ...
    if (q.data.startsWith("start_")) {
      ...
      const redirectLink = `https://shared-redirect.onrender.com/pay?bot=xposure&recipient=${TREASURY.toBase58()}&amount=0.01&reference=${reference.toBase58()}&userId=${userKey}`;
